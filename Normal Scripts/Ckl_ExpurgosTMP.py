...
token = os.getenv('Token_Ckl_ExpurgosTMP')
table_name = os.getenv('Table_Ckl_ExpurgosTMP')
                ...
                for i, id in enumerate(ids_to_register, start=1):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            HoraProblema = None
                            RegistradoPor = None
                            DataSolucionado = None
                            HoraSolucionado = None
                            FinalizadoPor = None
                            Observacao = None
                            ...
                            for categoria in categorias:
                                ...
                                for item in items:
                                    if item['name'] == 'DT':
                                        DT = item['answer']['number'] if item['answer'] else None
                                    elif item['name'] == 'Motivo':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            Motivo = item['answer']['selectedOptions'][0]['text']                                        
                                    elif item['name'] == 'Data do problema':
                                        DataProblema = item['answer']['text'] if item.get('answer') else None
                                        DataProblema = converter_data(DataProblema)  # Converter para YYYY-MM-DD
                                    elif item['name'] == 'Hora do problema':
                                        HoraProblema = item['answer']['text'] if item.get('answer') else None
                                    elif item['name'] == 'Data do problema':
                                        DataProblema = item['answer']['text'] if item.get('answer') else None
                                        DataProblema = converter_data(DataProblema)  # Converter para YYYY-MM-DD
                                    elif item['name'] == 'Hora do problema':
                                        HoraProblema = item['answer']['text'] if item.get('answer') else None
                                    elif item['name'] == 'Registrado por':
                                        RegistradoPor = item['answer']['text'] if item.get('answer') else None
                                    elif item['name'] == 'Data solucionado':
                                        DataSolucionado = item['answer']['text'] if item.get('answer') else None
                                        DataSolucionado = converter_data(DataSolucionado)  # Converter para YYYY-MM-DD
                                    elif item['name'] == 'Hora solucionado':
                                        HoraSolucionado = item['answer']['text'] if item.get('answer') else None                                                                                                                                                         
                                    elif item['name'] == 'Finalizado por':
                                        FinalizadoPor = item['answer']['text'] if item.get('answer') else None
                                    elif item['name'] == 'Observação':
                                        Observacao = item['answer']['text'] if item.get('answer') else None                                        

                            # Inserir os dados no banco de dados MySQL
                            sql = f"INSERT INTO {table_name} (id_Ckl, Status, Checklist, Unidade, Usuario, Assinatura, DT, Motivo, DataProblema, HoraProblema, RegistradoPor, DataSolucionado, HoraSolucionado, FinalizadoPor, Observacao) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                            val = (id, status, checklist_name, unidade_nome, usuario_nome, assinaturas, DT, Motivo, DataProblema, HoraProblema, RegistradoPor, DataSolucionado, HoraSolucionado, FinalizadoPor, Observacao)
