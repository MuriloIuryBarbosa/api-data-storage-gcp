...
token = os.getenv('Token_Ckl_Premiacao_Belem')
table_name = os.getenv('Table_Ckl_Premiacao_Belem')
                ...
                for i, id in enumerate(ids_to_register, start=1):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            Turno = None
                            SegurancaTipo = None
                            QuantidadeSeguranca = None
                            EficEOrganTipo = None
                            QuantidadeEficEOrg = None
                            AssiduidadeTipo = None
                            QuantidadeAssiduidade = None
                            ...
                            for categoria in categorias:
                                ...
                                for item in items:
                                    if item['name'] == 'Data':
                                        Data = item['answer']['text'] if item.get('answer') else None
                                        Data = converter_data(Data)  # Converter para YYYY-MM-DD
                                    if item['name'] == 'Turno':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            Turno = item['answer']['selectedOptions'][0]['text']
                                    if item['name'] == 'Segurança - Houve registro de acidente ou incidente no turno?':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            SegurancaTipo = item['answer']['selectedOptions'][0]['text']
                                    if item['name'] == 'Segurança - Quantidade de Ocorrências':
                                        QuantidadeSeguranca = item['answer']['number'] if item.get('answer') else None
                                    if item['name'] == 'Eficiência e Organização - Áreas de trabalho, máquinas, equipamentos e ferramentas estão conservados?':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            EficEOrganTipo = item['answer']['selectedOptions'][0]['text']
                                    if item['name'] == 'Eficiência e Organização - Quantidade de Ocorrências':
                                        QuantidadeEficEOrg = item['answer']['number'] if item.get('answer') else None
                                    if item['name'] == 'Assiduidade - Registro de faltas ou atrasos signicativos':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            AssiduidadeTipo = item['answer']['selectedOptions'][0]['text']
                                    elif item['name'] == 'Assiduidade - Quantidade de Ocorrências':
                                        QuantidadeAssiduidade = item['answer']['number'] if item.get('answer') else None

                            sql = f"INSERT INTO {table_name} (id_Ckl, Checklist, Unidade, Usuario, Status, Assinatura, Data, Turno, SegurancaTipo, QuantidadeSeguranca, EficEOrganTipo, QuantidadeEficEOrg, AssiduidadeTipo, QuantidadeAssiduidade) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                            val = (id, checklist_name, unidade_nome, usuario_nome, status, assinaturas, Data, Turno, SegurancaTipo, QuantidadeSeguranca, EficEOrganTipo, QuantidadeEficEOrg, AssiduidadeTipo, QuantidadeAssiduidade)
