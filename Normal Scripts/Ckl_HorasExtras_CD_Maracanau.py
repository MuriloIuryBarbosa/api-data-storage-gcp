...
token = os.getenv('Token_Ckl_HorasExtras_CD_Maracanau')
table_name = os.getenv('Table_Ckl_HorasExtras_CD_Maracanau')
                ...
                for i, id in enumerate(ids_to_register, start=1):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            MotivoHoraExtra = None 
                            Solicitante = None 
                            NomeSolicitanteGuizilim = None 
                            NomeSolicitanteSuzano = None 
                            Colaborador = None 
                            ExcederaLimiteHoraExtra = None 
                            QualMotivoParaExceder = None 
                            DataInicio   = None 
                            HoraInicio  = None 
                            DataFim   = None 
                            HoraFim  = None 
                            DataSource  = "Checklist Fácil" 

                            for categoria in categorias:
                                items = categoria.get('items', [])
                                for item in items:
                                    if item['name'] == 'Motivo da Hora Extra':
                                    ...
                                    if item['name'] == 'Nome do solicitante Guizilim':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            NomeSolicitanteGuizilim = item['answer']['selectedOptions'][0]['text']  
                                    if item['name'] == 'Nome do solicitante Suzano':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            NomeSolicitanteSuzano = item['answer']['selectedOptions'][0]['text']  

                                    if item['name'] == 'Data de início da Hora Extra':
                                        DataHoraExtra = item['answer']['text'] if item.get('answer') else None
                                        DataHoraExtra = converter_data(DataHoraExtra)
                                    if item['name'] == 'Data do fim da Hora Extra':
                                        DataFimDaHoraExtra = item['answer']['text'] if item.get('answer') else None
                                        DataFimDaHoraExtra = converter_data(DataFimDaHoraExtra)
                                    if item['name'] == 'Hora de início da Hora Extra':
                                        InicioHoraExtra = item['answer']['text'] if item.get('answer') else None
                                    if item['name'] == 'Hora de fim da Hora Extra':
                                        FimHoraExtra = item['answer']['text'] if item.get('answer') else None


                                    if item['name'] == 'Colaborador':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            Colaborador = item['answer']['selectedOptions'][0]['text']
                                    if item['name'] == 'O colaborador irá exceder o limite de 2 horas extras?':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            ExcederaLimiteHoraExtra = item['answer']['selectedOptions'][0]['text'] 
                                    if item['name'] == 'Motivo para exceder limite de Horas Extras':
                                        if item.get('answer') and item['answer'].get('selectedOptions'):
                                            QualMotivoParaExceder = item['answer']['selectedOptions'][0]['text']

                            # Inserir os dados no banco de dados MySQL
                            sql = f"INSERT INTO {table_name} (id_Ckl, Checklist, Unidade, Usuario, Status, Assinatura, MotivoHoraExtra, Solicitante, NomeSolicitanteGuizilim, NomeSolicitanteSuzano, Colaborador,  ExcederaLimiteHoraExtra, QualMotivoParaExceder, DataInicio, HoraInicio, DataFim, HoraFim, DataSource) VALUES (%s, %s, %s, %s, %s, %, %s, %s, %s, %s, %s, %)"
                            val = (id, checklist_name, unidade_nome, usuario_nome, status, assinaturas, MotivoHoraExtra, Solicitante, NomeSolicitanteGuizilim, NomeSolicitanteSuzano, Colaborador,  ExcederaLimiteHoraExtra, QualMotivoParaExceder, DataInicio, HoraInicio, DataFim, HoraFim, DataSource) 
