...
def processar_pagina(url, retries=5):
    for i in range(retries):
        response = requests.get(url, verify=False)
        if response.status_code == 200:
            evaluations_data = response.json()
            evaluation_ids = [str(evaluation['entries']) for evaluation in evaluations_data.get('data', [])]
...
def main():
    ...
    try:
        ...
        for checklist_id in checklist_ids:
            ...
            while True:
                url = f"https://api.umov.me/CenterWeb/api/25996e88384a5fb96e10edcaade9f19dbd5135/schedule.xml?serviceLocal=69187752&initialInsertDateTime=2024-07-01 00:00:00&finalInsertDateTime=2024-07-30 23:59:59&paging.page={page}"
                evaluation_ids = processar_pagina(url)
                ...
                for i, id in enumerate(ids_to_register, start=1):
                    try:
                        url_evaluation = f"https://api.umov.me/CenterWeb/api/25996e88384a5fb96e10edcaade9f19dbd5135/schedule/{id}.xml"
